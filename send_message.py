...
def compute_send_day(today: dt.date) -> int | None:
    """
    오늘 날짜(today)가 5/10/15/20를 주말·공휴일 보정한 '발송일'에 해당하면
    그 원래 기준일(5/10/15/20) 숫자를 반환. 아니면 None.
    """
...
7. 희망 색상/톤: 유나님이 알아서 해주세요 / XXX톤으로 해주세요 or 계절감이 느껴지는 색상으로 만들어주세요. 
8. 기한: xx월 xx일   오전(점심 전) or 제가 퇴근하기 전  까지 해주세요.
...
"""
    if day == 20:
        return f"""🗓️ {day}일 머니적립 출석체크 배너 제작 요청일입니다! 아래 텍스트를 채워 해당 채널에 보내주세요! 강유나 멘션은 필수입니다! ❤️
...
def send_for_day(day: int) -> None:
    ...
    if day == 10:
        text1 = f"""🗓️ {day}일 앱 푸시 배너 제작 요청일입니다! 아래 텍스트를 채워 해당 채널에 보내주세요! 강유나 멘션 필수입니다! ❤️

1. 제작할 내용 - 머니적립 or 금융광고 or 출석체크 
2. 매체사 - SKT / KT / LGU+
3. 배너 목적 – 이 배너에서 강조하고자 하는 것이 무엇인가요? 최종 목적은 무엇인가요? (낮은 이자 대출 홍보, 보험 가입, 카드 발급 등)
4. 타겟 사용자 – 이 배너는 어떤 사용자에게 노출되나요? 어떤 사람들에게 노출되는 건가요? (예: 대출 심사를 받은 50대 등)
5. 메인 문구 – 가장 강조해야 하는 문장을 써주세요. 키워드로 작성해도 좋아요
6. 보조 문구 – 보조적으로 쓰이는 문장은? 쓰셨던 텍스트 요약해서 작성해줘도 좋아요
7. 디자인 참고자료 – 디자인에서 피해갈 요소가 있나요? (예: 특정 색상, 문구 등)
8. 희망 색상/톤: 유나님이 알아서 해주세요 / XXX톤으로 해주세요 or 계절감이 느껴지는 색상으로 만들어주세요. 
9. 기한: xx월 xx일   오전(점심 전) or 제가 퇴근하기 전  까지 해주세요.
